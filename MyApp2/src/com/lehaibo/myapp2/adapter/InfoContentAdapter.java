...
import com.lehaibo.myapp2.cache.ImageLoaderEx;
...
public class InfoContentAdapter extends BaseAdapter {
	...
	public InfoContentAdapter(Context context, List<BaseType.InfoItem> data) {
		...
		mImageLoader.setDefaultBitmap(mContext.getResources().getDrawable(
				R.drawable.load_img));
	}
	...
	private View getBanner_0_View(int pos, View view) {
		...
		if (view == null) {
			view = LayoutInflater.from(mContext).inflate(
					R.layout.content_listitem_layout0, null);
	...
	private View getBanner_1_View(int pos, View view) {
		...
		if (view == null) {
			view = LayoutInflater.from(mContext).inflate(
					R.layout.content_listitem_layout1, null);
			holder = new ViewHolder1();
			holder.tvTitle = (TextView) view.findViewById(R.id.tv_title);
			holder.tvContent = (TextView) view.findViewById(R.id.tv_content);
			holder.ivContent = (ImageView) view.findViewById(R.id.iv_content);
			holder.tvImageCount = (TextView) view
					.findViewById(R.id.tv_imagecount);
			holder.tvArtist = (TextView) view.findViewById(R.id.tv_artist);
			view.setTag(holder);
		} else {
		...
		mImageLoader.DisplayImage(item.getThumnaiImageURL(0), holder.ivContent,
				mBusy);
	...
	private View getBanner_2_View(int pos, View view) {
		...
		if (view == null) {
			view = LayoutInflater.from(mContext).inflate(
					R.layout.content_listitem_layout2, null);
		...
		mImageLoader.DisplayImage(item.getThumnaiImageURL(0),
				holder.ivContent1, mBusy);
		mImageLoader.DisplayImage(item.getThumnaiImageURL(1),
				holder.ivContent2, mBusy);
		mImageLoader.DisplayImage(item.getThumnaiImageURL(2),
				holder.ivContent3, mBusy);
