...
import com.lehaibo.myapp2.base.ServerUrlBuilder;
...
public class ClientEngine {
	...
	public boolean httpGetRequestEx(BaseRequestPacket packet,
			IRequestDataPacketCallback callback) {
		...
		HttpResponseHandler handler = new HttpResponseHandler(packet.action,
				callback, null, packet.extra);
		client.get(packet.context, url, param, handler);

		return true;
	}

	public boolean httpPostRequestEx(BaseRequestPacket packet,
			IRequestDataPacketCallback callback) {
		String url = ServerUrlBuilder.getServerURL(packet.action);
		if (url.equals("")) {
			log.e("can't get serverURL by action : " + packet.action);
	...
	public boolean httpGetRequest(BaseRequestPacket packet,
		...
		HttpResponseHandler handler = new HttpResponseHandler(packet.action,
				null, callback, packet.extra);
		client.get(packet.context, url, param, handler);

		return true;
	}

	public boolean httpPostRequest(BaseRequestPacket packet,
			IRequestContentCallback callback) {
		String url = ServerUrlBuilder.getServerURL(packet.action);
		if (url.equals("")) {
			log.e("can't get serverURL by action : " + packet.action);
